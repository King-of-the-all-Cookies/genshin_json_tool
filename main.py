    ...
    def extract_voices(self):
        ...
        for folder in os.listdir(self.source_folder):
            ...
            for file_info in files_to_copy:
                ...
                source_path = os.path.join(folder_path, file_info["filename"])
                if os.path.exists(source_path):
                    file_info["source_path"] = source_path
                    self.log(f"Found {file_info['filename']} in {folder}")
        
        # Copy files and save JSON data
        self.log("\nProcessing files...")
        total_copied = 0
        char_stats = {}
        ...
        # Save JSON data for each character
